<?php
    class crud {
        ...
        public function editCat($id, $cname, $cdesc) {
            $sql = "UPDATE Cats SET catname=:cname, catdescription=:cdesc WHERE cat_id=:id";
            $stmt = $this->db->prepare($sql);
        ...
        public function editAcc($id, $aname, $aadd, $aemail, $aphone, $amrt) {
            $sql = "UPDATE Accommodation SET accname=:aname, address=:add, email=:em, phone=:ph, mrt=:mrt WHERE acc_id=:id";
            $stmt = $this->db->prepare($sql);
            
            // Bind parameters to the prepared statement
            $stmt->bindParam(':id', $id);
            $stmt->bindParam(':aname', $aname);
            $stmt->bindParam(':add', $aadd);
            $stmt->bindParam(':em', $aemail);
            $stmt->bindParam(':ph', $aphone);
            $stmt->bindParam(':mrt', $amrt);
